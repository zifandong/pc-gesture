...
NonameGesture.prototype.bindEventListener = function () {
  ...
  // this.element.addEventListener('wheel', this.handleWheel);//不需要滚轮事件
}
/**
 * 解绑事件
 */
...
/**
 * 获取图片缩放尺寸----------计算初始图片宽高以适应页面
 * @param {number} naturalWidth 图片自然宽度
 * @param {number} naturalHeight 图片自然高度
 * @param {number} maxWidth 最大显示宽度--window宽
 * @param {number} maxHeight 最大显示高度---window高
 * @returns
 */
